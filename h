...
-- Executor usage:
-- local NilApi = loadstring(game:HttpGet("<raw-url>"))()
-- local ui = NilApi.Create({ title = "My UI" })
-- local main = ui:CreateTab({ name = "Main", icon = "rbxassetid://..." })
-- local sec = main:CreateSection({ name = "Local Player", side = "Left" })
-- sec:AddToggle({ name = "Speed", default = false, callback = function(v) print(v) end })
...
local function safeParentGui()
    ...
    return playerGui or game:GetService("GuiService")
end
...
local function mkStroke(parent, thickness, transparency, color)
    local s = Instance.new("UIStroke")
    s.Thickness = thickness or 1
    s.Transparency = transparency == nil and 0.2 or transparency
    s.Color = color or Color3.fromRGB(40, 43, 46)
...
local function mkList(parent, padding)
...
local function bindHoverStroke(btn, stroke, accent)
    if not (btn and stroke) then
        return
    end
    if btn:GetAttribute("NilApiHoverBound") then
        return
    end
    btn:SetAttribute("NilApiHoverBound", true)

    local defaultColor = stroke.Color
    local defaultTr = stroke.Transparency
    local ti = TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    btn.MouseEnter:Connect(function()
        tween(stroke, ti, { Transparency = math.max(0, defaultTr - 0.15), Color = accent or defaultColor })
    end)
    btn.MouseLeave:Connect(function()
        tween(stroke, ti, { Transparency = defaultTr, Color = defaultColor })
    end)
end
...
local function createBaseWindow(opts)
    ...
    local guiParent = opts.parent
    if typeof(guiParent) ~= "Instance" then
        guiParent = safeParentGui()
    end

    local screenGui = Instance.new("ScreenGui")
    screenGui.Name = opts.guiName or ("NilApi_" .. randomName(14))
    ...
    mainFrame.Size = opts.size or UDim2.new(0, 560, 0, 390)
    mainFrame.BackgroundColor3 = Color3.fromRGB(23, 25, 28)
    mainFrame.BorderSizePixel = 0
    mainFrame.Parent = screenGui
    mkCorner(mainFrame, 14)
    mkStroke(mainFrame, 1, 0.2, Color3.fromRGB(40, 43, 46))
    ...
    dropShadow.ImageTransparency = 0.65
    ...
    titleLabel.Text = tostring(opts.title or "NilHub")
    ...
    closeBtn.BackgroundColor3 = Color3.fromRGB(27, 30, 33)
    ...
    local closeStroke = mkStroke(closeBtn, 1, 0.5, Color3.fromRGB(55, 58, 61))
    bindHoverStroke(closeBtn, closeStroke, accent)
    ...
    sideBar.Parent = body
    mkCorner(sideBar, 14)
    mkStroke(sideBar, 1, 0.35, Color3.fromRGB(40, 43, 46))
    ...
    do
        ...
        local startY = startPos.Y.Offset - (opts.initialDropOffsetY or 18)
        mainFrame.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset, startPos.Y.Scale, startY)
        ...
        for _, d in ipairs(mainFrame:GetDescendants()) do
            ...
            elseif d:IsA("Frame") then
                if d ~= mainFrame then
                    d.BackgroundTransparency = math.clamp(d.BackgroundTransparency + 0.4, 0, 1)
                end
            end
        end

        local tMove = TweenInfo.new(opts.initialDropTweenTime or 0.34, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
        tween(mainFrame, tMove, { Position = startPos, BackgroundTransparency = 0 })

        task.delay(opts.initialDropDelay or 0.02, function()
            local tFade = TweenInfo.new(opts.initialDropFadeTime or 0.30, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            for _, d in ipairs(mainFrame:GetDescendants()) do
                if d:IsA("TextLabel") or d:IsA("TextButton") then
                    tween(d, tFade, { TextTransparency = 0 })
                elseif d:IsA("ImageLabel") then
                    tween(d, tFade, { ImageTransparency = d.Name == "Shadow" and 0.65 or 0 })
                elseif d:IsA("Frame") then
                    -- leave original transparency alone if already set
                end
            end
        end)
    end
...
local function makeTabButton(accent, tabScroll, tabSpec)
    local b = Instance.new("TextButton")
    b.Name = tostring(tabSpec.name) .. "TabButton"
    b.Size = UDim2.new(1, 0, 0, 36)
    b.BackgroundColor3 = Color3.fromRGB(27, 30, 33)
    ...
    b.Parent = tabScroll
    mkCorner(b, 10)
    local st = mkStroke(b, 1, 0.5, Color3.fromRGB(55, 58, 61))
    bindHoverStroke(b, st, accent)
    ...
    icon.Image = tabSpec.icon or ""
    icon.ImageColor3 = Color3.fromRGB(230, 230, 230)
    icon.ImageTransparency = (tabSpec.icon and tabSpec.icon ~= "") and 0 or 1
    icon.Parent = b

    local lbl = Instance.new("TextLabel")
    lbl.Name = "Label"
    lbl.BackgroundTransparency = 1
    lbl.Position = UDim2.new(0, (tabSpec.icon and tabSpec.icon ~= "") and 34 or 12, 0, 0)
    ...
    lbl.Text = tostring(tabSpec.name)
    lbl.Parent = b
...
local function makeTabPage(parent)
    ...
    left.Position = UDim2.new(0, 0, 0, 10)
    left.Size = UDim2.new(0.5, -8, 1, -20)
    left.ScrollBarThickness = 0
    left.AutomaticCanvasSize = Enum.AutomaticSize.Y
    left.CanvasSize = UDim2.new(0, 0, 0, 0)
    left.Parent = page
    mkPadding(left, 6, 6, 0, 0)
    mkList(left, 12)
    ...
    right.Position = UDim2.new(0.5, 8, 0, 10)
    right.Size = UDim2.new(0.5, -8, 1, -20)
    right.ScrollBarThickness = 0
    right.AutomaticCanvasSize = Enum.AutomaticSize.Y
    right.CanvasSize = UDim2.new(0, 0, 0, 0)
    right.Parent = page
    mkPadding(right, 6, 6, 0, 0)
    mkList(right, 12)

    return page, left, right
end

local function makeSection(accent, parent, name, icon)
    local section = Instance.new("Frame")
    section.Name = tostring(name) .. "Section"
    ...
    mkCorner(section, 12)
    mkStroke(section, 1, 0.2, Color3.fromRGB(40, 43, 46))
    ...
    iconImg.ImageTransparency = (icon and icon ~= "") and 0 or 1
    iconImg.Parent = header

    local title = Instance.new("TextLabel")
    title.Name = "Title"
    title.BackgroundTransparency = 1
    title.Position = UDim2.new(0, (icon and icon ~= "") and 38 or 12, 0, 0)
    ...
    mkPadding(body, 10, 12, 12, 12)
    mkList(body, 10)

    return section, body
end

local function addLabelRow(body, text)
    ...
    row.TextSize = 14
    row.TextColor3 = Color3.fromRGB(160, 160, 165)
    row.Text = tostring(text or "")
    row.Parent = body
    return row
end

local function addDividerRow(body, text)
    local wrap = Instance.new("Frame")
    wrap.Name = "DividerRow"
    wrap.BackgroundTransparency = 1
    wrap.Size = UDim2.new(1, 0, 0, 22)
    wrap.Parent = body

    local lbl = Instance.new("TextLabel")
    lbl.Name = "DividerText"
    lbl.BackgroundTransparency = 1
    ...
    lbl.Text = tostring(text or "Settings")
    lbl.Parent = wrap

    local left = Instance.new("Frame")
    left.Name = "LeftLine"
    left.BackgroundColor3 = Color3.fromRGB(40, 43, 46)
    left.BorderSizePixel = 0
    left.AnchorPoint = Vector2.new(0, 0.5)
    left.Position = UDim2.new(0, 0, 0.5, 0)
    left.Size = UDim2.new(0, 84, 0, 1)
    left.Parent = wrap

    local right = Instance.new("Frame")
    right.Name = "RightLine"
    right.BackgroundColor3 = Color3.fromRGB(40, 43, 46)
    right.BorderSizePixel = 0
    right.AnchorPoint = Vector2.new(1, 0.5)
    right.Position = UDim2.new(1, 0, 0.5, 0)
    right.Size = UDim2.new(0, 84, 0, 1)
    right.Parent = wrap

    return wrap
end

local function addButtonRow(accent, body, spec)
    local row = Instance.new("Frame")
    row.Name = (spec.name or "Button") .. "Row"
    row.BackgroundTransparency = 1
    ...
    btn.BackgroundColor3 = Color3.fromRGB(27, 30, 33)
    btn.BorderSizePixel = 0
    btn.AutoButtonColor = false
    btn.Text = tostring(spec.name or "Button")
    btn.Font = Enum.Font.GothamBold
    btn.TextSize = 14
    btn.TextColor3 = Color3.fromRGB(230, 230, 230)
    btn.Parent = row
    mkCorner(btn, 10)
    local st = mkStroke(btn, 1, 0.4, Color3.fromRGB(55, 58, 61))
    bindHoverStroke(btn, st, accent)
...
local function addToggleRow(accent, body, spec)
    local row = Instance.new("Frame")
    row.Name = (spec.name or "Toggle") .. "ToggleRow"
    row.BackgroundTransparency = 1
    ...
    lbl.TextSize = 14
    lbl.TextColor3 = Color3.fromRGB(230, 230, 230)
    lbl.Text = tostring(spec.name or "Toggle")
    lbl.Parent = row
    ...
    box.BackgroundColor3 = Color3.fromRGB(27, 30, 33)
    box.BorderSizePixel = 0
    box.AutoButtonColor = false
    box.Text = ""
    box.Parent = row
    mkCorner(box, 6)
    local st = mkStroke(box, 1, 0.4, Color3.fromRGB(55, 58, 61))
    bindHoverStroke(box, st, accent)
    ...
    local function set(v, fire)
        ...
        tween(check, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            ImageTransparency = value and 0 or 1,
        })
        if fire and type(spec.callback) == "function" then
...
local function addSliderRow(accent, body, spec)
    local row = Instance.new("Frame")
    row.Name = (spec.name or "Slider") .. "SliderRow"
    row.BackgroundTransparency = 1
    ...
    lbl.TextSize = 14
    lbl.TextColor3 = Color3.fromRGB(230, 230, 230)
    lbl.Text = tostring(spec.name or "Slider")
    lbl.Parent = row

    local box = Instance.new("TextBox")
    box.Name = "ValueTextBox"
    box.AnchorPoint = Vector2.new(1, 0)
    box.Position = UDim2.new(1, -8, 0, -1)
    box.Size = UDim2.new(0, 64, 0, 20)
    box.BackgroundColor3 = Color3.fromRGB(27, 30, 33)
    box.BorderSizePixel = 0
    box.Text = ""
    box.TextColor3 = Color3.fromRGB(160, 160, 165)
    box.Font = Enum.Font.Code
    box.TextSize = 14
    box.ClearTextOnFocus = false
    box.Parent = row
    mkCorner(box, 8)
    local boxStroke = mkStroke(box, 1, 0.45, Color3.fromRGB(55, 58, 61))
    bindHoverStroke(box, boxStroke, accent)

    local trackArea = Instance.new("Frame")
    trackArea.Name = "SliderTrackArea"
    trackArea.BackgroundTransparency = 1
    trackArea.Position = UDim2.new(0, 0, 0, 22)
    ...
    mkCorner(track, 100)
    mkStroke(track, 1, 0.6, Color3.fromRGB(55, 58, 61))
    ...
    local changed = newSignal()
    local value = tonumber(spec.default)
    if value == nil then
        value = minV
    end
    ...
    local function setFromX(x)
        ...
        local r = math.clamp((x - absPos.X) / absSize.X, 0, 1)
        set(minV + (maxV - minV) * r, true)
    end
...
local function addDropdownRow(accent, body, spec)
    local row = Instance.new("Frame")
    row.Name = (spec.name or "Dropdown") .. "DropdownRow"
    row.BackgroundTransparency = 1
    row.Size = UDim2.new(1, 0, 0, 34)
    row.Parent = body
    ...
    btn.Text = "  " .. tostring(spec.name or "Dropdown")
    btn.Font = Enum.Font.GothamBold
    btn.TextSize = 14
    btn.TextColor3 = Color3.fromRGB(230, 230, 230)
    btn.Parent = row
    ...
    valueLbl.Size = UDim2.new(0, 140, 0, 18)
    valueLbl.TextXAlignment = Enum.TextXAlignment.Right
    valueLbl.TextYAlignment = Enum.TextYAlignment.Center
    valueLbl.Font = Enum.Font.Code
    valueLbl.TextSize = 14
    valueLbl.TextColor3 = Color3.fromRGB(160, 160, 165)
    valueLbl.Text = ""
    valueLbl.Parent = btn

    local listFrame = Instance.new("Frame")
    listFrame.Name = "DropdownList"
    listFrame.BackgroundColor3 = Color3.fromRGB(20, 22, 25)
    ...
    mkCorner(listFrame, 10)
    mkStroke(listFrame, 1, 0.35, Color3.fromRGB(40, 43, 46))
    ...
    local function set(v, fire)
        value = v
        row:SetAttribute("Value", tostring(value or ""))
        valueLbl.Text = tostring(value or "")
        if fire and type(spec.callback) == "function" then
            pcall(spec.callback, value)
        end
        if fire then
    ...
    local function rebuildList()
        ...
        for _, it in ipairs(items) do
            ...
            ib.BackgroundColor3 = Color3.fromRGB(27, 30, 33)
            ib.BorderSizePixel = 0
            ib.AutoButtonColor = false
            ib.Text = tostring(it)
            ib.Font = Enum.Font.GothamBold
            ib.TextSize = 14
            ib.TextColor3 = Color3.fromRGB(230, 230, 230)
            ib.Parent = scroll
            mkCorner(ib, 10)
            local is = mkStroke(ib, 1, 0.55, Color3.fromRGB(55, 58, 61))
            bindHoverStroke(ib, is, accent)
            ib.MouseButton1Click:Connect(function()
                set(it, true)
                listFrame.Visible = false
                tween(listFrame, TweenInfo.new(0.14, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Size = UDim2.new(1, 0, 0, 0) })
            end)
        end
    end
    ...
    local function setOpen(on)
        ...
        if open then
            ...
            local height = math.clamp(#items * 34 + 16, 60, 180)
            tween(listFrame, TweenInfo.new(0.16, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Size = UDim2.new(1, 0, 0, height) })
        else
            tween(listFrame, TweenInfo.new(0.14, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Size = UDim2.new(1, 0, 0, 0) })
...
local function addKeybindRow(accent, body, spec)
    local row = Instance.new("Frame")
    row.Name = (spec.name or "Keybind") .. "KeybindRow"
    row.BackgroundTransparency = 1
    row.Size = UDim2.new(1, 0, 0, 32)
    row.Parent = body
    ...
    lbl.TextSize = 14
    lbl.TextColor3 = Color3.fromRGB(230, 230, 230)
    lbl.Text = tostring(spec.name or "Keybind")
    lbl.Parent = row
    ...
    btn.Parent = row
    mkCorner(btn, 8)
    local st = mkStroke(btn, 1, 0.45, Color3.fromRGB(55, 58, 61))
    bindHoverStroke(btn, st, accent)

    local changed = newSignal()
    local current = spec.default
    ...
    local function setKey(key, fire)
        current = key
        row:SetAttribute("Value", current and current.Name or "")
        btn.Text = current and current.Name or "None"
        if fire and type(spec.callback) == "function" then
            pcall(spec.callback, current)
        end
        if fire then
...
function NilApi.Create(opts)
    local base = createBaseWindow(opts)
    ...
    local function setActive(tab)
        ...
        active = tab
        if active and active.Page then
            active.Page.Visible = true
        end
    end

    base.closeBtn.MouseButton1Click:Connect(function()
        if base.screenGui then
            base.screenGui.Enabled = false
        end
    end)
    ...
    function ui:GetGui()
        return base.screenGui
    end

    function ui:Show()
        base.screenGui.Enabled = true
    end
    ...
    function ui:Destroy()
        if base.screenGui then
            base.screenGui:Destroy()
        end
    end

    function ui:SetActiveTab(name)
        local t = tabs[tostring(name)]
        if t then
            setActive(t)
        end
    end

    function ui:CreateTab(tabSpec)
        tabSpec = tabSpec or {}
        local tabName = tostring(tabSpec.name or "Tab")
        if tabs[tabName] then
            return tabs[tabName]
        end

        local btn = nil
        btn = makeTabButton(base.accent, base.tabScroll, { name = tabName, icon = tabSpec.icon })
        local page, left, right = makeTabPage(base.pages)
        ...
        function tab:CreateSection(sectionSpec)
            sectionSpec = sectionSpec or {}
            local side = normalizeSide(sectionSpec.side)
            local parent = (side == "Right") and right or left
            local sec, body = makeSection(base.accent, parent, sectionSpec.name or "Section", sectionSpec.icon)
            local section = { Instance = sec, Body = body }

            function section:AddLabel(spec)
                return addLabelRow(body, (spec and spec.text) or spec)
            end

            function section:AddDivider(spec)
                if type(spec) == "table" then
                    return addDividerRow(body, spec.text)
                end
                return addDividerRow(body, spec)
            end

            function section:AddButton(spec)
                return addButtonRow(base.accent, body, spec or {})
            end

            function section:AddToggle(spec)
                return addToggleRow(base.accent, body, spec or {})
            end

            function section:AddSlider(spec)
                return addSliderRow(base.accent, body, spec or {})
            end

            function section:AddDropdown(spec)
                return addDropdownRow(base.accent, body, spec or {})
            end

            function section:AddKeybind(spec)
                return addKeybindRow(base.accent, body, spec or {})
            end
...
local function applyHoverStrokeTweens(screenGui, sideBar, bindButtonStrokeHover, bindTabScrollButtonHover)
...
local function apiCreatePageForTabImpl(pagesByTabText, pagesContainer, tabText)
...
local function apiCreateTabButtonImpl(sideBar, pagesByTabText, setActiveTab, bindTabScrollButtonHover, ACCENT_BLACK, ACCENT_PRIMARY, tabText, iconAsset)
    if not sideBar then
        return nil
    end
    ...
    -- Idempotent: if the tab already exists, reuse it.
    local existing = tabScroll:FindFirstChild(tabText .. "TabButton")
...
local function apiCreatePanelImpl(opts)
    opts = opts or {}
...
local function apiCreateSectionImpl(opts)
    opts = opts or {}
...
-- =========================
-- Builder helpers (API mode)
-- =========================

local function apiApplySheenGradient(target)
...
local function apiCreateToggleRowStyled(deps, opts)
    opts = opts or {}
    local rowName = tostring(opts.id or opts.name or "Toggle")
    local labelText = tostring(opts.text or opts.name or "Toggle")
    ...
    if deps and deps.bindButtonStrokeHover then
        deps.bindButtonStrokeHover(checkbox, checkboxStroke)
    end
    ...
    if deps and deps.bindCheckboxToggle then
        deps.bindCheckboxToggle(checkbox)
    end
    ...
    if hasKeybind then
        ...
        if deps and deps.bindKeybindButton then
            deps.bindKeybindButton(keybindButton)
        end

        keybindButton.Parent = toggleRow
    end

    return { Row = toggleRow, Checkbox = checkbox, KeybindButton = keybindButton }
end

local function apiCreateActionButtonStyled(deps, opts)
    opts = opts or {}
    local name = tostring(opts.id or opts.name or "ActionButton")
    local text = tostring(opts.text or opts.name or "Button")
    ...
    apiApplySheenGradient(button)
    if deps and deps.bindButtonStrokeHover then
        deps.bindButtonStrokeHover(button, stroke)
    end

    return button
end

local function apiCreateTextboxRowStyled(opts)
    opts = opts or {}
    local labelText = tostring(opts.label or opts.name or "Text")
    local textboxName = tostring(opts.id or opts.name or "Textbox")
    local placeholderText = tostring(opts.placeholder or "")
    ...
    textbox.Text = tostring(opts.default or "")
    ...
    return { Row = row, TextBox = textbox }
end

local function apiCreateSliderRowStyled(opts)
    opts = opts or {}
    local rowName = tostring(opts.id or opts.name or "Slider")
    local labelText = tostring(opts.text or opts.label or opts.name or "Slider")
    ...
    valueTextBox.PlaceholderText = tostring(minV)
    ...
    local currentValue = snapToStep(defaultValue)
    ...
    local function valueFromRatio(r)
        return snapToStep(minV + (maxV - minV) * r)
    end
    ...
    local function tweenHandleSize(targetSize)
        ...
        handleSizeTween = TweenService:Create(handle, handleSizeTweenInfo, { Size = targetSize })
    ...
    return {
        ...
        GetValue = function(_)
            return tonumber(sliderRow:GetAttribute("SliderValue"))
        end,
    }
end

local function apiCreateDropdownRowStyled(deps, opts)
    opts = opts or {}
    local labelText = tostring(opts.label or opts.name or "Dropdown")
    local defaultValue = tostring(opts.default or "[Select]")

    local dropdownRow = Instance.new("Frame")
    dropdownRow.Name = tostring(opts.id or "Frame")
    ...
    if deps and deps.bindButtonStrokeHover then
        deps.bindButtonStrokeHover(dropdownButton, buttonStroke)
    end
    ...
    dropdownRow:SetAttribute("DropdownValue", defaultValue)

    local popup = nil
    if deps and deps.createNilhubDropdownPopup then
        popup = deps.createNilhubDropdownPopup({})
    end
    ...
    if popup and popup.OnClosed then
    ...
    dropdownButton.MouseButton1Click:Connect(function()
        ...
        if popup.IsOpen and popup:IsOpen() then
        ...
        tweenArrow(180)
        popup:SetOptions(opts.options or {}, dropdownButton.Text, function(selected)
        ...
        task.delay(0.125, function()
            ...
            popup:Open(dropdownButton, dropdownButton, dropdownButton)
        end)
    end)

    return { Row = dropdownRow, Button = dropdownButton, Popup = popup }
end
...
function WindowMT:Tab(nameOrOpts, icon)
    if type(nameOrOpts) == "table" then
        return self:CreateTab(nameOrOpts)
    end
    return self:CreateTab({ name = nameOrOpts, icon = icon })
end

function WindowMT:CreateTab(opts)
    if type(opts) ~= "table" then
        return (self.CreateTab and self.CreateTab(opts))
    end
    ...
    local page = self.CreatePage and self.CreatePage(name)

    return setmetatable({ UI = self, Name = name, Page = page, _left = nil, _right = nil }, { __index = TabMT })
end

function TabMT:_ensurePanel(side)
    ...
    if side == "right" then
        if self._right and self._right.Parent ~= nil then
            return self._right
        end
        local existing = page:FindFirstChild("RightContentFrame1")
        if existing and existing:IsA("ScrollingFrame") then
            self._right = existing
            return existing
        end
        if self.UI.CreatePanel then
            self._right = self.UI.CreatePanel({ page = page, side = "right", name = "RightContentFrame1" })
        end
        return self._right
    end

    if self._left and self._left.Parent ~= nil then
        return self._left
    end
    local existing = page:FindFirstChild("LeftContentFrame1")
    if existing and existing:IsA("ScrollingFrame") then
        self._left = existing
        return existing
    end
    if self.UI.CreatePanel then
        self._left = self.UI.CreatePanel({ page = page, side = "left", name = "LeftContentFrame1" })
    end
    return self._left
end

function TabMT:CreateSection(opts)
    opts = opts or {}
    local panel = self:_ensurePanel(opts.side)
    if not panel then
        return nil
    end

    local sectionFrame, elements = self.UI.CreateSection({
        parent = panel,
        name = opts.name,
        title = opts.title,
        icon = opts.icon,
    ...
    return setmetatable({ UI = self.UI, Tab = self, Frame = sectionFrame, Body = elements }, { __index = SectionMT })
end

function TabMT:Left(nameOrOpts, icon)
    if type(nameOrOpts) == "table" then
        nameOrOpts.side = nameOrOpts.side or "Left"
        return self:CreateSection(nameOrOpts)
    end
    return self:CreateSection({ side = "Left", name = nameOrOpts, icon = icon })
end

function TabMT:Right(nameOrOpts, icon)
    if type(nameOrOpts) == "table" then
        nameOrOpts.side = nameOrOpts.side or "Right"
        return self:CreateSection(nameOrOpts)
    end
    return self:CreateSection({ side = "Right", name = nameOrOpts, icon = icon })
end

function SectionMT:AddToggle(opts)
    opts = opts or {}
    local deps = self.UI and self.UI._internals
    local built = apiCreateToggleRowStyled(deps, {
        id = opts.id,
        name = opts.name,
        text = opts.text,
        keybind = (opts.keybind == true) or (opts.hasKeybind == true),
    })
    built.Row.Parent = self.Body

    local defaultValue = (opts.default == true)
    if deps and deps.setCheckboxChecked then
        deps.setCheckboxChecked(built.Checkbox, defaultValue)
    end

    if type(opts.onKeybindChanged) == "function" and deps and deps.setKeybindChangedCallback and built.KeybindButton then
        deps.setKeybindChangedCallback(built.KeybindButton, opts.onKeybindChanged)
    end

    if type(opts.callback) == "function" then
        built.Checkbox:GetAttributeChangedSignal("AsteriaChecked"):Connect(function()
            opts.callback(built.Checkbox:GetAttribute("AsteriaChecked") == true)
        end)
        task.defer(function()
            opts.callback(defaultValue)
        end)
    end
...
function SectionMT:Toggle(nameOrOpts, defaultValue, callback)
    if type(nameOrOpts) == "table" then
        return self:AddToggle(nameOrOpts)
    end
    return self:AddToggle({ name = tostring(nameOrOpts), default = defaultValue, callback = callback })
end

function SectionMT:AddButton(opts)
    opts = opts or {}
    local deps = self.UI and self.UI._internals
    local btn = apiCreateActionButtonStyled(deps, opts)
...
function SectionMT:Button(nameOrOpts, callback)
    if type(nameOrOpts) == "table" then
        return self:AddButton(nameOrOpts)
    end
    return self:AddButton({ name = tostring(nameOrOpts), callback = callback })
end

function SectionMT:AddTextbox(opts)
    opts = opts or {}
    local built = apiCreateTextboxRowStyled(opts)
    built.Row.Parent = self.Body
    if type(opts.callback) == "function" then
        built.TextBox.FocusLost:Connect(function()
            opts.callback(built.TextBox.Text)
        end)
    end
    return built
end

function SectionMT:Textbox(nameOrOpts, defaultValue, callback)
    if type(nameOrOpts) == "table" then
        return self:AddTextbox(nameOrOpts)
    end
    return self:AddTextbox({ name = tostring(nameOrOpts), default = defaultValue, callback = callback })
end

function SectionMT:AddDropdown(opts)
    opts = opts or {}
    local deps = self.UI and self.UI._internals
    local built = apiCreateDropdownRowStyled(deps, opts)
    built.Row.Parent = self.Body
    return built
end

function SectionMT:Dropdown(nameOrOpts, options, defaultValue, callback)
    if type(nameOrOpts) == "table" then
        return self:AddDropdown(nameOrOpts)
    end
    return self:AddDropdown({ name = tostring(nameOrOpts), options = options, default = defaultValue, callback = callback })
end

function SectionMT:AddSlider(opts)
    opts = opts or {}
    local built = apiCreateSliderRowStyled(opts)
    built.Row.Parent = self.Body

    if type(opts.callback) == "function" then
        built.Row:GetAttributeChangedSignal("SliderValue"):Connect(function()
            opts.callback(tonumber(built.Row:GetAttribute("SliderValue")))
        end)
        task.defer(function()
            opts.callback(tonumber(built.Row:GetAttribute("SliderValue")))
        end)
...
function SectionMT:Slider(nameOrOpts, defaultValue, minV, maxV, stepV, callback)
    if type(nameOrOpts) == "table" then
        return self:AddSlider(nameOrOpts)
    end
    return self:AddSlider({
        name = tostring(nameOrOpts),
...
local function attachBuilder(controller)
    if not controller or type(controller) ~= "table" then
        return controller
    end
    if controller.__builderAttached then
        return controller
    end
    controller.__builderAttached = true

    -- Important: the controller already defines CreateTab/CreatePage/etc.
    -- Metatable __index won't override existing keys, so we wrap CreateTab
    -- to provide the builder behavior when passed a table.
    local rawCreateTab = controller.CreateTab
    local rawCreatePage = controller.CreatePage
    local rawCreatePanel = controller.CreatePanel
    local rawCreateSection = controller.CreateSection
    local rawSetActiveTab = controller.SetActiveTab
    local rawShow = controller.show or controller.Show
    local rawHide = controller.hide or controller.Hide
    local rawDestroy = controller.destroy or controller.Destroy
    local rawGetGui = controller.GetGui

    controller.__raw = controller.__raw or {}
    controller.__raw.CreateTab = controller.__raw.CreateTab or rawCreateTab
    controller.__raw.CreatePage = controller.__raw.CreatePage or rawCreatePage
    controller.__raw.CreatePanel = controller.__raw.CreatePanel or rawCreatePanel
    controller.__raw.CreateSection = controller.__raw.CreateSection or rawCreateSection
    controller.__raw.SetActiveTab = controller.__raw.SetActiveTab or rawSetActiveTab
    controller.__raw.Show = controller.__raw.Show or rawShow
    controller.__raw.Hide = controller.__raw.Hide or rawHide
    controller.__raw.Destroy = controller.__raw.Destroy or rawDestroy
    controller.__raw.GetGui = controller.__raw.GetGui or rawGetGui

    controller.CreateTab = function(self, optsOrName, iconAsset)
        if type(optsOrName) == "table" then
            local opts = optsOrName
            local name = tostring(opts.name or opts.text or "Tab")
            local icon = tostring(opts.icon or "")

            if rawCreateTab then
                rawCreateTab(name, icon)
            end
            local page = rawCreatePage and rawCreatePage(name)

            return setmetatable({ UI = self, Name = name, Page = page, _left = nil, _right = nil }, { __index = TabMT })
        end

        if rawCreateTab then
            return rawCreateTab(optsOrName, iconAsset)
        end
        return nil
    end
    ...
    controller.SetActiveTab = function(selfOrName, maybeName)
        local tabName = maybeName
        if tabName == nil then
            tabName = selfOrName
        end
        if rawSetActiveTab then
            return rawSetActiveTab(tostring(tabName or ""))
        end
        return nil
    end

    -- Convenience: allow ':' style for these too.
    ...
    controller.GetGui = function(self)
        if rawGetGui then
            return rawGetGui()
        end
        return self and self.screenGui
    end

    setmetatable(controller, { __index = WindowMT })
    return controller
end
...
-- Built-in gameplay binding: Local Player -> Speed toggle + Speed Value slider.
-- Kept outside createAsteriaGUI to avoid Luau local-register limits.
local function installLocalPlayerSpeedBinding(screenGui)
    if not (screenGui and typeof(screenGui) == "Instance") then
        return
    end

    local localPlayerSection = screenGui:FindFirstChild("Local PlayerSectionFrame", true)
    if not (localPlayerSection and localPlayerSection:IsA("Frame")) then
        return
    end

    if localPlayerSection:GetAttribute("AsteriaSpeedBound") == true then
        return
    end
    localPlayerSection:SetAttribute("AsteriaSpeedBound", true)

    local speedToggleRow = localPlayerSection:FindFirstChild("SpeedToggleRow", true)
    local speedCheckbox = speedToggleRow and speedToggleRow:FindFirstChild("Checkbox")
    if not (speedCheckbox and speedCheckbox:IsA("GuiButton")) then
        return
    end

    local speedSliderRow = localPlayerSection:FindFirstChild("Speed ValueSliderRow", true)
    if not (speedSliderRow and speedSliderRow:IsA("Frame")) then
        return
    end
    ...
    local function getEnabled()
        return speedCheckbox:GetAttribute("AsteriaChecked") == true
    end

    local function getTargetSpeed()
        local v = tonumber(speedSliderRow:GetAttribute("SliderValue"))
        if v == nil then
            v = 16
        end
    ...
    local function applySpeed(force)
        ...
        if not enabled then
            if force == true or humanoid.WalkSpeed ~= defaultWalkSpeed then
    ...
    -- Keep a game-accurate default when not enabled.
    do
        local humanoid = getHumanoid()
        if humanoid then
    ...
    table.insert(conns, speedCheckbox:GetAttributeChangedSignal("AsteriaChecked"):Connect(function()
        local humanoid = getHumanoid()
        if humanoid and not getEnabled() then
            defaultWalkSpeed = humanoid.WalkSpeed
        end
        applySpeed(true)
    end))

    table.insert(conns, speedSliderRow:GetAttributeChangedSignal("SliderValue"):Connect(function()
        if getEnabled() then
            applySpeed(false)
        end
    end))

    table.insert(conns, lp.CharacterAdded:Connect(function(character)
        if not bound then
            return
        end
    ...
    pcall(function()
        table.insert(conns, screenGui.Destroying:Connect(function()
            bound = false
            disconnectAll()
        end))
    end)
...
local function createAsteriaGUI(title, opts)
    ...
    local function _normalizeWhitelist(value)
    ...
    -- Optional performance controls (opt-in):
    -- - opts.defaultPagesWhitelist: {"Main","Settings"} or { Main=true }
    -- - opts.mainLeftSectionsWhitelist: {"Local Player"} etc.
    -- - opts.combatLeftSectionsWhitelist: {"Combat"} etc.
    -- - opts.buildMainRight=false / opts.buildCombatRight=false
    local defaultPagesWhitelist = _normalizeWhitelist(opts.defaultPagesWhitelist or opts.defaultPages)
    local mainLeftSectionsWhitelist = _normalizeWhitelist(opts.mainLeftSectionsWhitelist)
    local combatLeftSectionsWhitelist = _normalizeWhitelist(opts.combatLeftSectionsWhitelist)
    local buildMainRight = (opts.buildMainRight ~= false)
    local buildCombatRight = (opts.buildCombatRight ~= false)

    local function _wantPage(tabText)
        if not defaultPagesWhitelist then
            return true
        end
        return defaultPagesWhitelist[tostring(tabText)] == true
    end

    local function _wantSection(whitelist, title)
        if not whitelist then
            return true
        end
        return whitelist[tostring(title)] == true
    end
    ...
    local dropShadowDefaultTransparency = dropShadow.ImageTransparency

    -- Optional: mainframe intro animation (drop + fade in). This is separate from the
    -- module "row drop" animation and should run even in API-only mode.
    local playIntro = (opts.playIntro ~= false)
    local introPlayed = false
    local function playIntroAnimation()
        if introPlayed or not playIntro then
            return
        end
        introPlayed = true

        local startPos = mainFrame.Position
        local offsetY = tonumber(opts.introOffsetY or opts.initialDropOffsetY) or 18
        local introPos = UDim2.new(startPos.X.Scale, startPos.X.Offset, startPos.Y.Scale, startPos.Y.Offset - offsetY)

        mainFrame.Position = introPos
        mainFrame.BackgroundTransparency = 1
        stroke.Transparency = 1
        dropShadow.ImageTransparency = 1

        local tMove = TweenInfo.new(tonumber(opts.introTweenTime) or 0.34, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
        local tFade = TweenInfo.new(tonumber(opts.introFadeTime) or 0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)

        TweenService:Create(mainFrame, tMove, { Position = startPos }):Play()
        task.delay(tonumber(opts.introDelay) or 0.02, function()
            if mainFrame and mainFrame.Parent ~= nil then
                TweenService:Create(mainFrame, tFade, { BackgroundTransparency = mainFrameDefaultBgTransparency }):Play()
                TweenService:Create(stroke, tFade, { Transparency = mainFrameDefaultStrokeTransparency }):Play()
                TweenService:Create(dropShadow, tFade, { ImageTransparency = dropShadowDefaultTransparency }):Play()
            end
        end)
    end
    ...
    local function createMainLeftContentPanel()
        ...
        -- Local Player
        if _wantSection(mainLeftSectionsWhitelist, "Local Player") then
        ...
        -- Interactables
        if _wantSection(mainLeftSectionsWhitelist, "Interactables") then
    ...
    local function createCombatLeftContentPanel()
        ...
        -- Combat
        if _wantSection(combatLeftSectionsWhitelist, "Combat") then
        ...
        -- Voodoo
        if _wantSection(combatLeftSectionsWhitelist, "Voodoo") then
    ...
    if settingsPage then
        pagesByTabText.Settings = settingsPage
    end
    if mainPage then
        pagesByTabText.Main = mainPage
    end
    if farmPage then
        pagesByTabText.Farm = farmPage
    end
    if combatPage then
        pagesByTabText.Combat = combatPage
    end
    if tweenPage then
        pagesByTabText.Tween = tweenPage
    end
    if miscPage then
        pagesByTabText.Misc = miscPage
    end
    if inventoryPage then
        pagesByTabText.Inventory = inventoryPage
    end
    if webhookPage then
        pagesByTabText.Webhook = webhookPage
    end
    ...
    local function expandAllRowsForPage(page)
        ...
        for _, elementsContainer in ipairs(page:GetDescendants()) do
            if elementsContainer:IsA("ScrollingFrame") and elementsContainer.Name == "ElementsContainer" then
                sectionIndex += 1
                local sectionDelay = math.max(0, (sectionIndex - 1) * INITIAL_DROP_SECTION_STAGGER)
                ...
                if sectionFrame then
                    ...
                    if shouldShowSection then
                        sectionFrame.Visible = true
                        local originalSectionSize = decodeUDim2(sectionFrame:GetAttribute("AsteriaSearchOriginalSize")) or sectionFrame.Size
                        if sectionDelay > 0 then
                            task.delay(sectionDelay, function()
                                if sectionFrame and sectionFrame.Parent ~= nil then
                ...
                for _, child in ipairs(elementsContainer:GetChildren()) do
                    if child:IsA("GuiObject") then
                        ...
                        if originallyVisible then
                            local rowDelay = sectionDelay + math.max(0, (rowIndex - 1) * INITIAL_DROP_ROW_STAGGER)
                            if rowDelay > 0 then
    ...
    local function playInitialModuleDropAnimation()
        ...
        task.delay(INITIAL_DROP_DELAY, function()
            ...
            for i, p in ipairs(pages) do
                local pageDelay = math.max(0, (i - 1) * INITIAL_DROP_PAGE_STAGGER)
                if pageDelay > 0 then
    ...
    applyHoverStrokeTweens(screenGui, sideBar, bindButtonStrokeHover, bindTabScrollButtonHover)
    ...
    do
        -- Preserve ALL options so reload behaves the same as the initial build.
        -- Avoid table-copy loops here (this function is already near Luau's local-register limit).
        local reloadOpts
        if type(opts) == "table" then
            reloadOpts = opts
        else
        ...
        reloadOpts.title = title
        reloadOpts.buildTabs = buildTabs
        reloadOpts.buildDefaultContent = buildDefaultContent
        ...
        reloadUI = function()
            ...
            return attachBuilder(createAsteriaGUI(reloadOpts))
        end
    end
    ...
    local controller = {
        ...
        _internals = {
            bindButtonStrokeHover = bindButtonStrokeHover,
            bindCheckboxToggle = bindCheckboxToggle,
            setCheckboxChecked = setCheckboxChecked,
            bindKeybindButton = bindKeybindButton,
            setKeybindChangedCallback = function(button, cb)
                keybindChangedCallbackByButton[button] = cb
            end,
            createNilhubDropdownPopup = createNilhubDropdownPopup,
        },
        ...
        CreatePage = function(tabText)
            return apiCreatePageForTabImpl(pagesByTabText, pagesContainer, tabText)
        end,
        ...
        CreatePanel = function(panelOpts)
            return apiCreatePanelImpl(panelOpts)
        end,
        CreateSection = function(sectionOpts)
            return apiCreateSectionImpl(sectionOpts)
        end,
    }

    controller = attachBuilder(controller)

    -- Optional: allow the caller to finalize layout after everything exists.
    if opts and type(opts.onAfterBuild) == "function" then
        pcall(function()
            opts.onAfterBuild(controller)
        end)
    end

    -- Optional: pick an initial tab after build (helps prevent a "blank" page).
    if opts and opts.initialActiveTab ~= nil then
        local initialTab = tostring(opts.initialActiveTab or "")
        if initialTab ~= "" then
            pcall(function()
...
function Api.Create(arg)
    if type(arg) == "table" then
        ...
        local gui = attachBuilder(createAsteriaGUI(opts))
        if gui and gui.screenGui then
            pcall(installLocalPlayerSpeedBinding, gui.screenGui)
        end
        return gui
    end

    local gui = attachBuilder(createAsteriaGUI(arg or "Asteria Hub", {
        buildTabs = false,
...
function Api.new(opts)
    ...
    function controller:Reload()
        ...
        gui = attachBuilder(createAsteriaGUI(createOpts))
        if gui and gui.screenGui then
            pcall(installLocalPlayerSpeedBinding, gui.screenGui)
        end
        return gui
